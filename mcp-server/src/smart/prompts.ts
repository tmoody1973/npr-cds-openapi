...
// Rules every prep follows: the page shows tool details and rights notes itself, and a station never builds a segment on someone's worst day.
const PREP_RULES = `- Do not mention tools, tool names, or how the tools compare stations.
...
- THE WEEK IN REVIEW: only the stories marked new, grouped by show or collection, each as a headline, one sentence, the canonical link, and the audio length if there is audio. Then one line with the count of stories marked only updated, in this form: "13 older stories were updated; nothing new in them."
- WHAT WE MISSED: from step 2, the NPR stories not marked localized, each with one sentence on the local angle and its canonical link. If a subject returned no NPR stories, say so in one line and move on.
...
2. Pick the single guest or subject from step 1 most likely to have coverage elsewhere in the network. Call coverage_scan with topic=<that>, since="${since}". If it returns nothing, run coverage_scan once more with the show's genre or its collection name, then stop searching.
3. Call find_stories with station="NPR", query=<the words from step 2 that returned stories, or the first words if neither did>, since="${since}", limit=10. NPR's own stories on it.
