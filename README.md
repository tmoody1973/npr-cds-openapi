...
`mcp-server/` is published to npm as **`npr-cds-mcp`** (current version 0.8.4). It is an MCP server, a small program that lets an AI assistant such as Claude use CDS as a tool. You type a question in plain English; the assistant picks a tool, the tool talks to CDS, and the answer comes back as a list a producer can read out loud.
...
| `find_station` | a name, call letters, or city | station id and name, with city, state, band and music-only when the finder knows it |
| `find_collection` | a topic, tag, show, or program name | its id |

Four saved prompts, `morning-prep`, `weekly-prep`, `show-prep` and `newsletter-draft`, chain these tools into a workflow the assistant follows the same way every time; your MCP client lists them next to the tools. The generated tools (`queryDocuments`, `getDocument`, and the rest, one per CDS endpoint) are still there for the full document when you need it.
