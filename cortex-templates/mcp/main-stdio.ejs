...
import { CONFIG_DIR, saveHomeStation, smartInstructions } from './smart/config';
...
async function main() {
  ...
  // createServer() (generated by cortex from the spec) sets generic instructions at construction;
  // the SDK has no public way to change them afterwards, so reach into the underlying Server the
  // same way the smart layer overrides other generated behavior. Upgrade to a public setter if the
  // SDK ever adds one (see @modelcontextprotocol/sdk server/index.js: `this._instructions`).
  (server.server as any)._instructions = smartInstructions();
